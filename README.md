...
## Chapter Something 2 - COFF Parsing (Common Object File Format)

COFF parsing does happen in this project, and there was never really a choice about it. Every Windows portable executable carries a COFF file header inside it, sitting immediately after the PE signature, so the moment you parse a PE at all you are parsing COFF whether you set out to or not. That part was settled before I ever thought about it.

Linux is a different story, and this is the part I had wrong. Linux does not use COFF. It used a.out in the very early days and moved to ELF, which is what it has used ever since. COFF was AT&T's format for Unix System V back in the early 1980's, and most of the commercial Unix world eventually moved off it to ELF in much the same way. It has not vanished completely, since AIX still uses a variant called XCOFF, but on a modern Linux machine you are looking at ELF. COFF's living relatives are Windows object files, Windows static libraries, and that header sitting inside every single PE.

So the question was never whether to parse COFF. It was whether to accept a standalone COFF file as something you can hand to exi directly, meaning an object file (`.obj` on Windows, `.o` on Unix) or a static library (`.lib` on Windows, `.a` on Unix, both of which are `ar` archives with a pile of object files packed inside them). After a lot of going back and forth in my own head, I opted not to.

Object files are the manuscript before it becomes a book. Every symbol is named, including the internal ones that were never meant to leave the building. Every address that still needs patching is written out as a relocation. MSVC leaves a `.drectve` section behind with instructions for the linker, and a pair of `.debug$S` and `.debug$T` sections carrying CodeView type and local variable information, which is there even in cases where no PDB was ever produced. Then the linker takes the whole pile, resolves the symbols against one another, assigns real addresses, and throws most of the paperwork away. What comes out the other side is the executable, and it knows considerably less about itself than the objects that produced it did.

Here is where I had it backwards. I originally wrote that loading an `.obj` into exi would give you more or less the same information as the COFF header already sitting inside a PE. It would not, and it is not close. The 20-byte file header is the same structure in both, which is the part that made me think it in the first place, but an object file also carries a full symbol table, per-section relocations, and a stack of compiler paperwork that the linked executable simply does not have anymore. If anything an object file tells you more than a finished binary does, and if you follow that thought all the way through it is an argument for parsing them rather than against.

The actual argument is that you will not have one. An analyst runs into an object file in two situations. The first is on a build server, where that output is exactly what you would expect to find and where whoever is looking already has the source sitting right there. The second is because somebody shipped one by accident. That second one does happen, it is just not something you can reasonably build a feature around, and anyone capable of writing the malware is capable of working out what handing over their symbol table would cost them. There is a third case worth being honest about, which is that static libraries turn up constantly in SDKs and are full of COFF objects, but a `.lib` is a build input rather than something that ever ran on somebody's machine.

I also said in an earlier draft that doing this would be a few more lines of code, and that was me being generous with myself. Doing it properly means the `ar` archive format for `.lib` files, the full COFF symbol table along with its string table, per-section relocations, and COMDAT handling for the function-level sections MSVC likes to emit. It also means a second shape for the model entirely, because an object file has no image layout at all. No entry point, no ImageBase, no RVAs, nothing that was ever going to be mapped anywhere. The `Binary` type in this project assumes a thing that could be loaded, and an object file is not that thing.

So COFF gets parsed where it actually lives, which is inside the PE, and that is where it stops. What I do want is for exi to recognize an object file when one turns up and say so plainly, rather than failing somewhere deep inside a header with a message that helps nobody. Detecting it and declining it politely is a couple of lines. Parsing it is a project.

If you genuinely need to go digging inside an object file, the tools for it are already good and I saw no reason to write another one. `dumpbin /symbols /headers` ships with Visual Studio. `llvm-readobj` and `llvm-objdump` both understand COFF and run anywhere. GNU binutils `objdump` handles PE/COFF as well. For taking a `.lib` apart, `lib.exe` or `llvm-ar` will list and extract the members for you.

## Artifical Intelligence Notice

Aritifical Intelligence was used in various aspects during the creation of this program. Primarily, it helped to organize, scaffold, and fix various syntax and spelling errors made in the code. Secondly, artifical inteligence was used as a platform for learning. Most of what was learned and demonstrated came from lesson plans created by artifical intelligence. External studies were conducted and will be cited properly as the project finishes up. Third, artifical intelligence was used for code generation and revision. Lastly, artificial intelligence was heavily used to adapt the Wikipages and markdown files to a more professional apperance. 

Artificial intelligence is such a fantastic tool. However, it cannot yet create an application from start to finish without issues. Larger projects also suffer as a result of this. Since it was used in this project in a few different ways, it would only be appropriate to mention that it was used and how it was used. 